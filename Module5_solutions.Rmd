...
```{r exercise_1_nls_exp_soln, echo = FALSE}
...
<p> For the biexponential, we need initial guesses for $a_1$ and $a_2$, $r_1$ and $r_2$. We first took $a_1$ and $a_2$ equal to half of the fitted $a$ value above, and $r_1=r_2=-0.014$, again, from the fit above. The starting point faisl to returna value. We then decrease $r_2$ to $-0.05$:   </p>

```{r exercise_1_nls_biexp_soln, echo = FALSE}
...
<p> Next, we try applying gradient descent. First we declare the gradient descent function as in Module 5: <p/>
...
```{r exercise-1-grad-descent_exp-soln, echo = TRUE}
x0 = c(5.5,-0.01)

gd = grad.descent(determine_sse_exp,x0,step.size=0.00002,max.iter=4000, #400000 after knitting
                        stopping.deriv=0.01)
...
and call the gradient descent function:

```{r exercise-1-grad-descent-biexp_soln, echo = TRUE}
x0 = c(3,-0.03,3,-0.06)
#x0 = c(3,-0.1,3,-0.01)
gd = grad.descent(determine_sse,x0,step.size=0.00001,max.iter=1000, #100000 after knitting
                        stopping.deriv=0.01)
x0_out <- c(gd$x, determine_sse(gd$x))
x0_out
plot(BE_data)
...
The result (-158.9178, 91.53143) is in close agreement with the result for Example 9.
...
library(GA)
ga1 <- ga(type = "real-valued", fitness = function(x) -determine_sse_ex3(c(x[1],x[2], x[3], x[4])),
         lower = c(0.1,0.1,0.1,0.1), upper = c(20,10,5,5), maxiter = 25) #250 after knitting
```
...
```{r ex4_plot-rejection-histograms, echo=FALSE}
...
We begin by definig the SSE function:
...
Plotting the posterior distributions, we see mean values of about $\beta \approx 14$, $\gamma \approx 1.3$.
