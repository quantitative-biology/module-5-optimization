...
```{r, echo=FALSE, out.height="50%", fig.cap="Extreme Values", fig.alt="A graph showing local maxima and minima on a function."}
...
```{r, echo=FALSE, out.height="50%", fig.cap="Graph of $f(x) = x^2+3x-2$ (Example 1) ", fig.alt="Graph of $f(x) = x^2+3x-2$ shown on an axis."}
...
```{r, echo=FALSE, out.height="50%", fig.cap="Graph of $f(x) = 3x^4-4x^3-54x^2+108x$  (Example 2) ", fig.alt="Graph of $f(x) = 3x^4-4x^3-54x^2+108x$ shown on an axis."}
...
```{r, echo=FALSE, out.height="50%", fig.cap="Finding a line of best fit through the origin (Example 3)", fig.alt="Defining error between points and other points that rest on the line of best fit."}
...
```{r plot-iris-dataset, fig.cap="Dataset: Sepal Length vs Petal Width", fig.alt = "Graphical representation of 'Dataset: Sepal Length vs Petal Width'.", echo = TRUE}
...
```{r plot-linear_regression_iris_dataset, fig.cap="Dataset: Sepal Length vs Petal Width, with best-fit line", fig.alt = "Graphical representation of 'Dataset: Sepal Length vs Petal Width' with a line of best fit.", echo = TRUE}
plot(iris$Sepal.Length~iris$Petal.Width, xlab = "Petal Width", ylab = "Sepal Length")
...
```{r plot-Michaelis-Menten-dataset, fig.cap="Dataset: reaction velocity against substrate concentration", fig.alt = "Graphical representation of 'Dataset: reaction velocity against substrate concentration' which contain points following a logarithmic function.", echo = TRUE}
...
```{r plot_nonlinear_regression_Michaelis-Menten_dataset, fig.cap="Dataset: reaction velocity against substrate concentration and best-fit model", fig.alt = "Graphical representation of 'Dataset: reaction velocity against substrate concentration' with a line of best fit showing a logarithmic pattern.",echo = TRUE}
plot(MM_data)
...
```{r plot-wireframe-convex-function, fig.cap="Surface plot of a paraboloid", fig.alt="Surface plot of paraboloid shown on a square domain.",echo=FALSE,message=FALSE}
...
```{r plot-wireframe-complicated-function, fig.cap="Surface plot with multiple minima", fig.alt="A surface plot with multiple minima plotted on a square domain.", echo=FALSE,message=FALSE}
...
```{r plot-surface-complicated-function,fig.cap="Interactive surface plot with multiple minima", fig.alt="An interactive version of a surface plot with multiple minima plotted on a square domain.", echo=FALSE,message=FALSE,warning=FALSE}
library(MASS)
library(plotly)
x <- seq(-4, 4, length= 30)
y <- seq(-4, 4, length= 30)
...
```{r gradient-descent-plot-paths-complicated-function, fig.cap="Surface plot with multiple minima and search paths", fig.alt="A downwards perspective of a surface plot with multiple minima plotted on a square domain with search paths following several points.", echo=FALSE}
...
```{r plot-wireframe-egg-carton-function, fig.cap="Surface plot with many minima, resembling an egg carton", fig.alt="A surface plot with many minima, resembling an egg carton in a square domain.", echo=FALSE}
f.name <- "Example 9"
...
```{r plot-egg-carton-simulated-annealing-traces,fig.cap="Progress of simulated annealing algorithm", fig.alt="The progress of simulated annealing algorithm in a graph.", echo=FALSE}
...
```{r report-summary-GA, fig.cap="Progress of genetic algorithm", fig.alt="This graph shows that the genetic algorithm matches the solution by displaying the mean and the line of best fit (which both have a similar exponential trend).",echo=TRUE}
summary(ga)
```
...
```{r simulate-and-plot-lotka-volterra, fig.cap="Simulation of Lotka-Volterra system.", fig.alt="A graph comparing time and population density between predator and prey.", echo = TRUE, warning = FALSE}
library(deSolve)
...
```{r generate-and-plot-lotka-volterra-corrupted-data, fig.cap="Data for Lotka-Volterra system.", fig.alt="The plots used for Figure 20.", echo = FALSE, warning = FALSE}
...
```{r plot-lotka-volterra-best-fit-and-data, fig.cap="Data for Lotka-Volterra system and best-fit simulation.", fig.alt="Combination of the Lotka-Volterra system and best-fit simulation.", echo = FALSE, warning = FALSE}
...
```{r exercise-3-generate-lotka-volterra-corrupted-data, fig.cap="Data for Lotka-Volterra system for exercise 3", fig.alt="Plots on a graph representing predator and prey population density vs. time.", echo = FALSE, warning = FALSE}
...
```{r plot-prediction-confidence-intervals-linear-regression-iris, fig.cap="Prediction confidence intervals of linear regression fit.", fig.alt="Plots on a graph with x-axis 'Sepal Length' and y-axis 'Petal Width' and a line of best fit with two additional lines representing the parameters of the confidence intervals.", echo=TRUE}
...
```{r 2plot-Michaelis-Menten-dataset, fig.cap="Nonlinear regression fit from example 5", fig.alt="Plots with a logarithmic line of best fit.", echo = TRUE}
#substrate concentrations
S <- c(4.32, 2.16, 1.08, 0.54, 0.27, 0.135, 3.6, 1.8, 0.9, 0.45, 0.225, 0.1125, 2.88, 1.44, 0.72, 0.36, 0.18, 0.9, 0)
#reaction velocities
V <- c(0.48, 0.42, 0.36, 0.26, 0.17, 0.11, 0.44, 0.47, 0.39, 0.29, 0.18, 0.12, 0.50, 0.45, 0.37, 0.28, 0.19, 0.31, 0)
...
```{r plot-rejection-histograms, fig.cap="Posterior histograms for Michaelis-Menten fit.", fig.alt="Histogram demonstrating Km vs. Frequency.", echo=FALSE}
...
```{r exercise-1-dataset, fig.cap="Exercise 1 showing decreasing drug concentration", fig.alt="Drug concentration in blood over time as plots that follow a decreasing trend.", echo = TRUE}
...
```{r exercise_1_nls_exp_soln, fig.cap="Exercise 1 data decreasing drug concentration with expoential fit.", fig.alt="Fitted exponential line of best fit as the solution.", echo = FALSE}
...
```{r exercise_1_nls_biexp_soln, fig.cap="Exercise 1 data decreasing drug concentration with biexponetial fit.", fig.alt="Fitted biexponential line of best fit as the solution.", echo = FALSE}
...
```{r exercise-1-grad-descent_exp-soln, fig.cap="Exercise 1 data decreasing drug concentration with expoential fit gradient descent", fig.alt="Fitted exponential with gradient descent, as the line of best fit, as the solution.", echo = TRUE}
x0 = c(5.5,-0.01)
...
```{r exercise-1-grad-descent-biexp_soln, fig.cap="Exercise 1 data decreasing drug concentration with biexponetial fit gradient descent", fig.alt="Fitted biexponential with gradient descent, as the line of best fit, as the solution", echo = TRUE}
x0 = c(3,-0.03,3,-0.06)
...
```{r ex4_plot-rejection-histograms, fig.cap="Exercise 4 posterior histograms", fig.alt="These figures show histograms that show very small bars for the x-direction and wider spread bars for the y-direction.", echo=FALSE}
c1 <- rgb(173,216,230,max = 255, alpha = 80, names = "lt.blue")
c2 <- rgb(255,192,203, max = 255, alpha = 80, names = "lt.pink")
