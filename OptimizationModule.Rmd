...
grad.descent = function(f, x0, step.size=0.05, max.iter=200,
                        stopping.deriv=0.01, ...) {
# record the number of parameters to be estimated 
#  (i.e. the dimension of the parameter space)
    n = length(x0)  
 # initialize a matrix to record the sequence of estimates
  xmat = matrix(0,nrow=n,ncol=max.iter)
  # the first row of matrix xmat is the initial 'guess'
  xmat[,1] = x0   
  for (k in 2:max.iter) { #loop over the allowed number of steps
     # Calculate the gradient 
      #(a vector indicating steepness and direct of greatest ascent)
...
grad.descent = function(f, x0, step.size=0.05, max.iter=200, 
                        stopping.deriv=0.01, ...) {
  # record the number of parameters to be estimated (i.e. the dimension of the parameter space)
  n = length(x0)  
  # initialize a matrix to record the sequence of estimates
  xmat = matrix(0,nrow=n,ncol=max.iter) 
