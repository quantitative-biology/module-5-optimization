...
<p> Below is a table showing the x values and the objective value derived by gradient descent starting from those four points. We could see the results are pretty close to each other. </p>
...
<p> The table below shows the endpoints for each of these four search paths, all of which are roughly identical, and reach a roughly equivalent objective value. </p>
...
<p> Here we see that from some initial guess values, the algorithm successfully reaches the global minimum from others, it gets stuck at a local minimum. </p>
...
<p> Below is a table showing the x values and the objective value derived by gradient descent starting from those five points. We could see the results are different with different initial guesses. </p>
...
For this example, we will estimate the parameters $p_2$ and $p_3$.
...
#### Exercise 3:
We will now try and estimate the parameters $p_1$, $p_2$, $p_3$ and $p_4$ as well as enter the predictions into the `GenSA` command in order to minimize the sum of squares error value. 
...
Inputting the predicted values of $p_1$, $p_2$, $p_3$ and $p_4$ into the `GenSA` function, gives us the parameters 29.246138, 4.879013, 2.018677 and 6.058124. These parameters minimize the sum of squared errors value to 4.989519. 
