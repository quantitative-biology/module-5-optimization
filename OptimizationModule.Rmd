...
sample_by_rejection <- function (true_data, n_iterations, acceptance_threshold, accept_or_reject_function) {
  ...
  return(data.frame(cbind("accepted_or_rejected" = accepted_or_rejected, 
                          "sampled_Ks" = sampled_K, "sampled_Vms" = sampled_Vm)))
}
```

Here we randomly sample $K$ and $V_m$ 20000 times, it can be seen from the histogram that the values that occur most frequently are close to the estimated values derived from the non linear regression model.

```{r,echo=FALSE}
set.seed(132)
sampled_parameter_values_squared_distances = sample_by_rejection(v, 20000, 0.001, accept_or_reject_with_squared_distance)

hist(sampled_parameter_values_squared_distances[which(sampled_parameter_values_squared_distances$accepted_or_rejected==1),2],xlab = "K", main="Distribution for K")
hist(sampled_parameter_values_squared_distances[which(sampled_parameter_values_squared_distances$accepted_or_rejected==1),3],xlab = "Vm", main="Distribution for Vm")
```
